...
"""codr - handy way to launch VS Code with a given directory.

Does a fuzzy (substring) search, if any subdirectory contains the search term,
will open VS Code in that directory.

Works with Python 3.6+ and must have the VS code shell command set up (open the
...
specified in ~/.codr.json, which is a JSON file containing two keys: basepaths
...
def get_paths():
    ...
    result = [
        Match(f.name, f) for p in data["basepaths"] for f in p.iterdir() if f.is_dir()
